...
func (r *Resolver) Build(target resolver.Target, cc resolver.ClientConn, opts resolver.BuildOptions) (resolver.Resolver, error) {
	...
	ctx, _ := context.WithTimeout(context.Background(), CONST_DURATION_GRPC_TIMEOUT_SECOND)
	prefix := GetPrefix(instance.schema, instance.serviceName)
	resp, err := instance.cli.Get(ctx, prefix, clientv3.WithPrefix())
	if err == nil {
		var addrList []resolver.Address
		for i := range resp.Kvs {
			addrList = append(addrList, resolver.Address{Addr: string(resp.Kvs[i].Value)})
		}
		instance.cc.UpdateState(resolver.State{Addresses: addrList})
		instance.watchStartRevision = resp.Header.Revision + 1
		go instance.watch(prefix, addrList)
	} else {
...
func serviceNameFromTarget(target resolver.Target) string {
	...
	endpoint, _ = url.PathUnescape(endpoint)
